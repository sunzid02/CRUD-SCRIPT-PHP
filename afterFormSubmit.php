    ...
    if ($hostDb1!=null && $usernameDb1!=null &&  $nameOfdb1 !=null && $hostDb2!=null &&  $nameOfdb2!=null && $tableName!=null && $criteria!=null)
    {
      ...
      if ($criteria == "1")
      {
        //////////////////////////////////////////////////Criteria 1//////////////////////////////////////////////////////////////////////////////////
                ...
                $chunkData = array_chunk($dynamicDataentry,$limit,true);
                $dynamicDataentrySize = sizeof($dynamicDataentry);
                // echo $dynamicDataentrySize."<br>";
                print_r($chunkData);

                $chunkDataSize = sizeof($chunkData);
                // echo "<br>";
                // $qryInsertDb2 = "insert into ".$tableName." ".$headerFormatDb2." values ";
                ...
                {
                  $queryIns = "insert into ".$tableName." ".$headerFormatDb2." values ";
                  $queryIns .= implode(", ",$chunkData[$i]);
                  echo $queryIns."<br>";

                  $readyForInsert[] = $queryIns;
                }

                print_r($readyForInsert);
                // $readyForInsertSize = sizeof($readyForInsert);
                //Insert chunk data finally
                  foreach ($readyForInsert as $key)
                  {
                    if (mysql_query($key) == true)
                    {
                      echo "<br>Insert success: ". $key."<br>";
                    }
                    else
                    {
                      echo "<br>Insert Unsuccessfull ". $key."<br>".mysql_error();
                      // exit(mysql_error());
                    }
                  }
        ...
        if (is_array($et))
        {
          ...
          // die();
            ...
            if ($emptyQryDb2Ex2 == true)
            {
              ...
              $qryInsertDb2 = "insert into  ".$tableName." ".$headerDb22." values ";
              $valuesArray = array();
              foreach ($et as $key)
              {
                $valuesArray[]=$key;
              ...
              for ($i=0; $i <$chunkDataSize ; $i++)
              {
                $queryIns = "insert into ".$tableName." ".$headerArrayDb2." values ";
                $queryIns .= implode(", ",$chunkData[$i]);
                echo $queryIns."<br>";
